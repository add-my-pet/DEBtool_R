diso_221b_var = function(t, var, tXT, p) {
...
if(is.complex(M_E1) == TRUE | M_E1 < 0){
    M_E1 = 1e-10;
}
if(is.complex(M_E2) == TRUE | M_E2 < 0){
    M_E2 = 1e-10;
}

# get environmental variables at age t (linear interpolation)
X1 = unlist(spline1(t, tXT[,c(1,2)], 0, 0)[1]); # mol/cd^2, food density of type 1
X2 = unlist(spline1(t, tXT[,c(1,3)], 0, 0)[1]); # mol/cd^2, food density of type 1 
T  = unlist(spline1(t, tXT[,c(1,4)], 0, 0)[1]); # K, temperature 
...
# reserve dynamics
sgr_iso_21_var_out =          # 1/d, specific growth rate, ....
    unlist(sgr_iso_21_var(m_E1, m_E2, j_E1S, j_E2S, mu_E1, mu_E2, mu_V, k_E, kap_G, kap)); # use continuation
r=sgr_iso_21_var_out[1]; j_E1_S=sgr_iso_21_var_out[2]; j_E2_S=sgr_iso_21_var_out[3]; j_E1C=sgr_iso_21_var_out[4]; j_E2C=sgr_iso_21_var_out[5]; j_E1P=sgr_iso_21_var_out[6]; j_E2P=sgr_iso_21_var_out[7];
...
if(X1 == 0){
 pyield = y_E1X2/(y_E1X2 + y_E2X2);
 cyield = y_E2X2/(y_E1X2 + y_E2X2);
}else{
 pyield = y_E1X1/(y_E1X1 + y_E2X1);
 cyield = y_E2X1/(y_E1X1 + y_E2X1);
}

 injest1 = dcM_X1 * y_E1X1 / (1-y_P1X1) + dcM_X2 * y_E1X2 / (1-y_P2X2);
 injest2 = dcM_X1 * y_E2X1 / (1-y_P1X1) + dcM_X2 * y_E2X2 / (1-y_P2X2);
 assim1 = j_E1A * M_V;
 assim2 = j_E2A * M_V;
 mobil1 = J_E1C;
 mobil2 = J_E2C;
 grow_tiss1 = (r + j_V_S) * M_V;
 grow_tiss2 = (r + j_V_S) * M_V;
 grow_ohead1 = (r + j_V_S)/ y_VE1 * M_V - (r + j_V_S) * M_V;
 grow_ohead2 = (r + j_V_S)/ y_VE2 * M_V - (r + j_V_S) * M_V;
 maint1 = j_E1_S * M_V;
 maint2 = j_E2_S * M_V;
 mat_maint1 = k_J * E_H * 0.5 / mu_E1;
 mat_maint2 = k_J * E_H * 0.5 / mu_E2;
 maturation1 = ((1 - kap) * mu_E1 * J_E1C  - (k_J * E_H) * 0.5) * (E_H < E_Hp) / mu_E1;
 maturation2 = ((1 - kap) * mu_E2 * J_E2C  - (k_J * E_H) * 0.5) * (E_H < E_Hp) / mu_E2;
 repro_tiss1 = kap_R1 * J_E1R * (E_H >= E_Hp);
 repro_tiss2 = kap_R2 * J_E2R * (E_H >= E_Hp);
 repro_ohead1 = (1 - kap_R1) * J_E1R * (E_H >= E_Hp);
 repro_ohead2 = (1 - kap_R2) * J_E2R * (E_H >= E_Hp);
 lost_rejres1 = (1-kap_E1) * j_E1P * M_V;
 lost_rejres2 = (1-kap_E2) * j_E2P * M_V;
 ret_rejres1 = kap_E1 * j_E1P * M_V;
 ret_rejres2 = kap_E2 * j_E2P * M_V;
 dres1 = assim1 - mobil1 + ret_rejres1;
 dres2 = assim2 - mobil2 + ret_rejres2;
 faeces1 = injest1 - assim1;
 faeces2 = injest2 - assim2;
 totretain1 = grow_tiss1 + repro_tiss1 + dres1;
 totretain2 = grow_tiss2 + repro_tiss2 + dres2;
 grow_repro1 = grow_tiss1 + repro_tiss1;
 grow_repro2 = grow_tiss2 + repro_tiss2;
 test1 = mobil1 - grow_tiss1 - grow_ohead1 - maint1 - mat_maint1 - maturation1 - repro_tiss1 - repro_ohead1 - ret_rejres1 - lost_rejres1;
 test2 = mobil2 - grow_tiss2 - grow_ohead2 - maint2 - mat_maint2 - maturation2 - repro_tiss2 - repro_ohead2 - ret_rejres2 - lost_rejres2;

# pack output
dvar = c(pyield, cyield, injest1, injest2, assim1, assim2, mobil1, mobil2, grow_tiss1, grow_tiss2, grow_ohead1, grow_ohead2, maint1, maint2, mat_maint1, mat_maint2, maturation1, maturation2, repro_tiss1, repro_tiss2, repro_ohead1, repro_ohead2, lost_rejres1, lost_rejres2, ret_rejres1, ret_rejres2, dres1, dres2, faeces1, faeces2,  totretain1, totretain2, grow_repro1, grow_repro2);
