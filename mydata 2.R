...
Dir="C:\\Users\\Documents\\GitHub\\Kooijman Rainbow trout"

gw124=read.table(paste(Dir, "\\gw124.txt", sep=""), sep="\t", dec=".", header=T)
gw124$study="gw124"
gw124$tanks_path=gw124$condition
gw124=gw124[, which(names(gw124) %in% c("condition", "tanks_path", "study", "date", "weight"))]
gw150=read.table(paste(Dir, "\\gw150.txt", sep=""), sep="\t", dec=".", header=T)
gw150$study="gw150"
gw150=gw150[, which(names(gw150) %in% c("condition", "tanks_path", "study", "date", "weight"))]
gw124bvar=read.table(paste(Dir, "\\gw124bvar.txt", sep=""), sep="\t", dec=".", header=T)
gw124bvar$study="gw124bvar"
gw124bvar=gw124bvar[, which(names(gw124bvar) %in% c("condition", "tanks_path", "study", "date", "weight"))]



# Take out last date for gw150 because strange
gw150=gw150[gw150$date<300,]

# all in one dataset
gw=rbind(gw124, gw150, gw124bvar)
summary(gw)
gw=gw[-which(is.na(gw$weight)),]

# Call all the functions


Dir='C:/Users/Owner/Dropbox/post doc/Modelisation/RBT growth/R/DEB models/Kooijman Rainbow trout'


setwd(Dir)
names.files=list.files(Dir)
for (i in c(1:length(names.files))){
  if (!(names.files[i] %in% c("mydata.R", "mer.R", "mydata 2.R", "verifs.R", "verifs 2.R"))){
...
# Initiate results
results=data.frame(ah=NA, ab=NA, ap=NA, am=NA, Lb=NA, Lp=NA, Li=NA, Wi=NA, Ri=NA, v=NA, kap=NA, kap_R=NA, p_Mvol=NA, p_Tsurfa=NA, k_J=NA, kap_G=NA, condition=NA, study=NA, tanks_path=NA)

for (tanks_path in unique(gw$tanks_path)){
  ...
  temp=gw[gw$tanks_path == tanks_path,]
  tW=as.matrix(temp[, which(names(temp) %in% c("date", "weight"))])
...
#  sd = 0 * pars[[1]][,1]                                  # initiate standard deviations
  ...
  merr = mre('predict_Oncorhynchus_mykiss', pars[[1]], Data, tW)  # WLS-method
  FIT = 10 * (1 - merr)  # get mark for goodness of fit 
  
  # get predictions
  
  tspan = data.frame(0:1500)  # times for plotting length data
  predict=predict_Oncorhynchus_mykiss(pars[[1]][,1], Data, tspan)  # notice use of first column of pars only
  ...
  # Save the data
  temp_res=c(predict[[1]], as.character(unique(temp$condition)), unique(temp$study), unique(tanks_path))
...
# results$condition[results$condition==1]="BPA0.1"
# results$condition[results$condition==2]="BPA0.3"
# results$condition[results$condition==3]="BPA3"
# results$condition[results$condition==4]="BPA30"
# results$condition[results$condition==5]="BPA300"
# results$condition[results$condition==6]="control"

library(sciplot)
for (var in names(results)[c(1:16)]){
  results[, var]=as.numeric(results[, var])
  temp=results[, which(names(results) %in% c(var, "condition"))]
  names(temp)[names(temp)==var]="var"
  bargraph.CI(condition, var, data=temp, main=var)
}

results=results[-which(results$condition=="BPA300"),]
mod=lm(p_Mvol~condition, data=results)
anova(mod)
