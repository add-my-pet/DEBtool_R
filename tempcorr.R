tempcorr = function (Temp,T_1,Tpars) {
  ...
  if (length(Tpars) == 1) {TC = exp(T_A/T_1 - T_A/Temp) }
  else if (length(Tpars) == 3) {
    T_L = Tpars(2)  # Lower temp boundary
    T_AL = Tpars(3)  # Arrh. temp for lower boundary
    TC = exp(T_A/ T_1 - T_A/ Temp) * (1 + exp(T_AL./ T_1 - T_AL/ T_L)) /   (1 + exp(T_AL/ Temp - T_AL/ T_L)) 
  }
  else {
    T_L = Tpars(2)  # Lower temp boundary
  ...
  TC = exp(T_A/ T_1 - T_A/ Temp) * (1 + exp(T_AL/ T_1 - T_AL/ T_L) + exp(T_AH/ T_H - T_AH/ T_1))/
