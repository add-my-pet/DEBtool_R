iso_221_var = function(tXT, var_0, p, n_O, n_M, r0) {
  
source('DEBtoolR/lib/misc/spline1.R'); source('DEBtoolR/lib/misc/tempcorr.R'); source('DEBtoolR/iso221/diso_221_var.R')
...
# get variables
diso_221_var_out<-ode(var_0, tXT[,1], diso_221_var, tXT=tXT, p=p, method="ode45") # note first col returned is y, unlike in MATLAB
t=diso_221_var_out[,1]; var=as.matrix(diso_221_var_out[,2:14]);
...
s1 = apply(cbind(0, 1 - m_E1/ m_E1m),1,max); s2 = apply(cbind(0, 1 - m_E2/ m_E2m),1,max);
...
flux = as.matrix(cbind(f1, f2, s1, s2, rho_X1X2, rho_X2X1)); 

return(list(var,flux))    
