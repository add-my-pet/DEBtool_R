iso_21_var <- function(m_E1b, m_E2b, p){
library(pracma)
# [L_b a_b M_E10 M_E20 info] = iso_21_var(m_E1b, m_E2b, p)
# created 2012/03/06 by Bas Kooijman, R version created 2015/06/03
#
...
#
# requires fniso_21_var, diso_21_var (defined immediately below - must come first unlike in MATLABs)

## subfunction fniso_21_var

fniso_21_var <- function(M_E0, m_E1b, m_E2b, p){
...
LEa<-ode(c(L_0, m_E10, m_E20, a_0),c(E_H0,E_Hb), diso_21_var, p, method="ode45" ) # note first col returned is y, unlike in MATLAB
F = c(m_E1b, m_E2b) - LEa[nrow(LEa),3:4]; # mol/mol, norm function set to zero

L_b = LEa[nrow(LEa),2]; # cm, length at birth
a_b = LEa[nrow(LEa),5]; # d, age at birth

#return(as.matrix(c(F[1], F[2], L_b, a_b)))
return(F)
}
  

fniso_21_var2 <- function(M_E0, m_E1b, m_E2b, p){
# for use by fsolve in iso_21: F = 0 if M_E0 are such that m_Eb are specified values
...
LEa<-ode(c(L_0, m_E10, m_E20, a_0),c(E_H0,E_Hb), diso_21_var, p, method="ode45" ) # note first col is y, ulike in MATLAB
F = c(m_E1b, m_E2b) - LEa[nrow(LEa),3:4]; # mol/mol, norm function set to zero

L_b = LEa[nrow(LEa),2]; # cm, length at birth
a_b = LEa[nrow(LEa),5]; # d, age at birth

return(list(F, L_b, a_b))
}  
## subfunction diso_21_var

diso_21_var <- function(E_H, LEa, p){
# odes for iso_21:
...
sgr_iso_21_var_out =                       # cm/d, change in L, ..
    as.numeric(sgr_iso_21_var(m_E1, m_E2, j_E1M, j_E2M, mu_E1, mu_E2, mu_V, v/L, kap_G, kap));
  r=sgr_iso_21_var_out[1]; j_E1_M=sgr_iso_21_var_out[2]; j_E2_M=sgr_iso_21_var_out[3]; j_E1C=sgr_iso_21_var_out[4]; j_E2C=sgr_iso_21_var_out[5]; j_E1P=sgr_iso_21_var_out[6]; j_E2P=sgr_iso_21_var_out[7];
...
dLEa = c(dL, dm_E1, dm_E2, 1)/ dE_H;                           # pack output
return(list(dLEa))
}    
...
M_E0  = fsolve(f=fniso_21_var, x0=c(M_E10, M_E20), tol = 1e-15, m_E1b=m_E1b, m_E2b=m_E2b, p=p)$x;
M_E10 = M_E0[1]; M_E20 = M_E0[2];
# get L_b and a_b
fniso_21_var_out = fniso_21_var2(M_E0, m_E1b, m_E2b, p);
F=fniso_21_var_out[1]; L_b=fniso_21_var_out[2]; a_b=fniso_21_var_out[3];
  
return(list(L_b, a_b, M_E10, M_E20))
}